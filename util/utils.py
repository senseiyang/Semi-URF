...
class DiceLoss(nn.Module):
    ...
    def _dice_loss(self, score, target, ignore):
        ...
        loss = 1 - loss
        return loss
    def _mse_loss(self, score, target, ignore):
        target = target.float()
        loss = torch.mean((score[ignore != 1] - target[ignore != 1])**2)
        return loss
...
class MSELoss(nn.Module):
    ...
    def forward(self, inputs, target, weight=None, softmax=False, ignore=None):
        ...
        loss = 0.0
        for i in range(0, self.n_classes):
            mse_ = self._mse_loss(inputs[:, i], target[:, i], ignore)
            class_wise_mse.append(mse_.item())
            loss += mse_ * weight[i]
        return loss / self.n_classes
